...
int BinaryTree<T>::height() {
    if(root_) return root ->height(root_);
    return -1;
}
...
int BinaryTree<T>::leaves() {
    int leaves_right, leaves_left;
    leaves_left = leaves_right = 0;
    if(node->left_) leaves_left = leaves(node->left_);
    if(node->rigth_) leaves_right = leaves(node -> rigth_);
...
ArrayList<T> BinaryTree<T>::limits() {
    ArrayList<T> L(2);
    Node *t =  root_ -> minimum();
    Node *t2 = root_ -> maximum();
    L.push(t);
    L.push(t2);
    return L;
}
...
BinaryTree<T> BinaryTree<T>::clone() {
    BinaryTree<T> C;
    ArrayList<T> pre_order = this -> pre_order();
    for(Node* a: pre_order) {
        C.push(a);
    }
    return C;
}
...
void BinaryTree<T>::filter(int n_child) {
    ArrayList<T> pos_ordem = this-> post_order();
    for (int i = 0; i < pos_ordem>max_size(); i++) {
        Node* filtering = post_order.at(i)
        if (n_child == 0) {
            if (!filtering->left && !filtering->right) {
                this -> remove(filtering->data());
            }
        } else if (n_child == 1) {
            if (filtering -> left && filtering -> right) {
                this -> remove(filtering-> data());
            } else if (!filtering->left && filtering->rigth) {
                this -> remove(filtering-> data());
            }
        } else if (n_child == 2) {
            if (filtering -> left && filtering->right) {
